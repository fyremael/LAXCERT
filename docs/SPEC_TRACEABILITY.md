...
| --- | --- | --- | --- | --- | --- |
| 1 | A candidate JSON can be emitted from LAXFORGE. | Complete for MVP. | LAXFORGE draft PR `fyremael/LAXFORGE#4` adds `scripts/export_laxcert_calibration.py`; exported `runs/laxcert_calibration_pr` certified through `certify_candidate_input` with `status: proof_succeeded`; `proof-status.json` records `source_kind: laxforge_artifact_dir` and manifest hash. | None for MVP; evidence quality improves when the PR is merged and CI supplies the artifact path. | Preserve optional external artifact CI gate. |
...
- LAXFORGE PR `https://github.com/fyremael/LAXFORGE/pull/4` now supplies a real producer-side calibration exporter. The artifact generated by that PR certified successfully through the same external input path.
...
- LAXFORGE PR `fyremael/LAXFORGE#4` was updated to emit `laxcert_schema_version`; its generated artifact still certifies with `status: proof_succeeded`.

Verification:

```bash
pytest -q
python -m laxforge_laxcert.emitter_contract F:\_codex\LAXFORGE\runs\laxcert_calibration_pr --repo-root .
```
